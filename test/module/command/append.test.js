...
test('An append command executes a parser function', () => {
  ...
  let request = command.parser(['append', 'foo', '0', '120', '2', 'noreply']);
  expect(request).toBeInstanceOf(Request);
  expect(request.cmd).toBe('append');
  ...
  const request01 = command.parser(['append']);
...
test('An append command executes a validate function', () => {
  ...
  expect(command.validate([['append', 'foo', '0', '120', '2']], true)).toBeUndefined();
  expect(command.validate([['append', 'foo', '0', '120', '2', 'noreply']], true)).toBeUndefined();
  expect(command.validate([['append', 'foo', '0', '120', '2', 'noreply'], 'ab'], false)).toBeUndefined();
  expect(command.validate([['append', 'foo', '0', '120']], true)).toBeInstanceOf(Error.Default);
  expect(command.validate([['append', 'foo', 'A', '120', '2', 'noreply']], true)).toBeInstanceOf(Error.Client);
  expect(command.validate([['append', 'foo', '0', 'B', '2', 'noreply']], true)).toBeInstanceOf(Error.Client);
  expect(command.validate([['append', 'foo', '0', '120', 'C', 'noreply']], true)).toBeInstanceOf(Error.Client);
  expect(command.validate([['append', 'foo', '0', '120', '2', 'noreply'], 'abc'], false)).toBeInstanceOf(Error.Client);
});
