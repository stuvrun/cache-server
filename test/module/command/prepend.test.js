...
test('An prepend command executes a parser function', () => {
  ...
  let request = command.parser(['prepend', 'foo', '0', '120', '2', 'noreply']);
  expect(request).toBeInstanceOf(Request);
  expect(request.cmd).toBe('prepend');
  ...
  const request01 = command.parser(['prepend']);
...
test('An prepend command executes a validate function', () => {
  ...
  expect(command.validate([['prepend', 'foo', '0', '120', '2']], true)).toBeUndefined();
  expect(command.validate([['prepend', 'foo', '0', '120', '2', 'noreply']], true)).toBeUndefined();
  expect(command.validate([['prepend', 'foo', '0', '120', '2', 'noreply'], 'ab'], false)).toBeUndefined();
  expect(command.validate([['prepend', 'foo', '0', '120']], true)).toBeInstanceOf(Error.Default);
  expect(command.validate([['prepend', 'foo', 'A', '120', '2', 'noreply']], true)).toBeInstanceOf(Error.Client);
  expect(command.validate([['prepend', 'foo', '0', 'B', '2', 'noreply']], true)).toBeInstanceOf(Error.Client);
  expect(command.validate([['prepend', 'foo', '0', '120', 'C', 'noreply']], true)).toBeInstanceOf(Error.Client);
  expect(command.validate([['prepend', 'foo', '0', '120', '2', 'noreply'], 'abc'], false)).toBeInstanceOf(Error.Client);
});
